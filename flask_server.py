...
from server_utils import gen_VAD_frames_wrapper, CustomJSONizer
...
class FlaskServer(object):
    def __init__(self, host='0.0.0.0', port=5000) -> None:
        ...
        def job_one_step():
            ...
            # self.gen_VAD_frames = gen_VAD_frames_wrapper(
            #     self.results_dict['video_anomaly_detector']['results'])
        ...
        def VAD_video_feed():
            ...
            # return Response(self.gen_score_frame(),
            #                 mimetype='multipart/x-mixed-replace; boundary=frame')
            # return f'data:image/png;base64,{self.gen_score_frame()}'
            return Response(
                self.gen_frame(),
                # self.VAD_frame,
                # mimetype='image/png'
        ...
        def get_anomaly():
            ...
            for model_name, data in self.results_dict.items():
                ...
                results_dict[model_name]['results'] = list(map(
                    lambda result: {
                        'type': result['type'], 'anomaly': result['anomaly'], 'pred': result['pred'], 'sensors': result['sensors']},
                    data['results'])
                )
