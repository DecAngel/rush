...
from models.PVCGN.lib import metrics
from models.PVCGN.lib.metrics import MAPE2
    ...
    for horizon_i in range(horizon):
        ...
        if save_res:
            if horizon_i == 0:
                truth = y_truth
                preds = y_pred
                np.save('data/xian/test/result/truth.npy', truth)
                np.save('data/xian/test/result/preds.npy', preds)
...
class StepLR2(MultiStepLR):
    ...
    def get_last_lr(self):
        lr_candidate = super(StepLR2, self).get_last_lr()
...
def main(args):
    ...
    device = torch.device(
        'cuda', index=3) if torch.cuda.is_available() else torch.device('cpu')
    ...
    # adjacency matrices are concatenated according to the last dimension
    ...
    for epoch in range(cfg['train']['epochs']):
        ...
        for _, (x, y, xtime, ytime) in enumerate(train_iterator):
            ...
            loss = criterion(y_pred, y) + criterion_mape(y_pred, y, 10, device) * 100
            loss.backward()
