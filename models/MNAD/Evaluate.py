...
labels = np.load(
    '/home/yuanyu/projects/rush/models/MNAD/data/frame_labels_'+args.dataset_type+'.npy')
...
for video in sorted(videos_list):
    ...
    if args.method == 'pred':
        labels_list = np.append(
            labels_list, labels[5+label_length:videos[video_name]['length']+label_length])
    else:
        labels_list = np.append(
            labels_list, labels[label_length:videos[video_name]['length']+label_length])
    label_length += videos[video_name]['length']
    psnr_list[video_name] = []
    feature_distance_list[video_name] = []
print(f'labels_list length: {len(labels_list)}')
label_length = 0
video_num = 0
...
for k, (imgs) in enumerate(test_batch):
    ...
    if args.method == 'pred':
        ...
        mse_imgs = loss_func_mse((outputs[0]+1)/2, (imgs[0, 3*4:]+1)/2)
    ...
    psnr_list[videos_list[video_num].split('/')[-1]].append(psnr(mse_loss))
...
print('calculate anomaly scores'.center(100, '-'))
for video in sorted(videos_list):
    video_name = video.split('/')[-1]
    print(video_name, len(psnr_list[video_name]))
    print(psnr_list[video_name])
    # anomaly_score_dict[video_name] = score_sum(anomaly_score_list(psnr_list[video_name]),
    #                                            anomaly_score_list_inv(feature_distance_list[video_name]), args.alpha)
    anomaly_score_dict[video_name] = anomaly_score_list(psnr_list[video_name])
    print(anomaly_score_dict[video_name])
    anomaly_score_total_list += anomaly_score_dict[video_name]
